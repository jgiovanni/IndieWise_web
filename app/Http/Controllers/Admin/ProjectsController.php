...
//        $this->middleware('jwt.refresh', ['only' => ['store', 'update', 'destroy']]);
    ...
    {
        $watched = DB::select('SELECT * FROM `Watched` w LEFT JOIN (SELECT p.id, p.name AS projectName, p.url_id AS projectUrlId, p.thumbnail_url AS projectThumbnail FROM projects p ) as project ON project.id = w.project_id WHERE (w.project_id IN ( SELECT Project.id FROM projects WHERE Project.unlist = false ) AND w.count > 0 ) ORDER BY w.updated_at DESC LIMIT 7');
