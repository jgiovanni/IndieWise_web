...
angular.module("videosharing-embed", []), angular.module("videosharing-embed").service("PlayerConfig", function () {
    ...
    var d = {
        youtube: {
            type: "youtube",
            settings: {autoplay: 0, controls: 1, loop: 0},
        ...
        vimeo: {
            type: "vimeo",
            settings: {autoplay: 0, loop: 0, api: 0, player_id: ""},
        ...
        vine: {
            type: "vine",
            settings: {audio: 0, start: 0, type: "simple"},
            ...
            g.$observe("width", function (a) {
                ...
                if (void 0 !== f && f !== h) {
                    ...
                    if (angular.forEach(b, function (a) {
                        ...
                        }), null === i)return void e.onChange();
                    {
                        var j = f.match(i.playerRegExp), k = j[2], l = j[1], m = f.match(i.timeRegExp);
                        i.config
                    }
                    if (angular.forEach(a("whitelist")(g, i.whitelist), function (a, b) {
                            var c = void 0 != i.transformAttrMap[b] ? i.transformAttrMap[b] : b;
                            i.settings[c] = a
                        }), i.settings.start = 0, m)switch (i.type) {
                        case"youtube":
                            i.settings.start += 60 * parseInt(m[2] || "0") * 60, i.settings.start += 60 * parseInt(m[4] || "0"), i.settings.start += parseInt(m[6] || "0");
                            break;
                        case"dailymotion":
                            i.settings.start += parseInt(m[1] || "0")
                    }
                    if (i.isAdditionaResRequired())for (var n = angular.element(d.document.querySelector("body")), o = 0; o < i.additionalRes.length; o++) {
                        var p = i.additionalRes[o];
                        null == d.document.querySelector("#" + p.id) && n.append(p.element)
                    }
                    e.onChange({videoId: k, provider: i.type});
