<template>
	...
	<div class="large-8 columns" v-if="critique">
		...
		<!-- Comments -->
		<comments ref="comments" :comments="comments" :parent="critique" :disable="false" :child="false"></comments>
		<!-- End Comments -->
...
<script type="text/babel">
	...
	export default{
		...
		data(){
			return {
				critique: null,
				comments: {data: [], meta: {pagination: {current_page: 0}}},
				pagination: null,
		...
		methods: {
			loadComments(page) {
				// Fetch Comments
				this.$http.get('comments', { params: {
					critique: this.critique.id,
					per_page: 10,
					page: page,
					replies: false,
					sort: this.sort
					...
					this.$refs.comments.loading = false;
		...
		mounted(){
			...
			this.$root.$on('loadComments', function (page) {
