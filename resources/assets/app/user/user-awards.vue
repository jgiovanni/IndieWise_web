			...
			<section class="content">
				...
				<div class="row">
					<div class="large-12 columns" v-if="nominations && nominated && awards">
						<md-tabs md-alignment="fixed" md-dynamic-height>
							<md-tab id="allnominations"
							        :md-label="(nominations.data.length + nominated.data.length) +  ' Nominations'">
								<md-tabs md-alignment="fixed" md-dynamic-height>
									<md-tab id="nominations" :md-label="nominations.data.length + ' Nominations Given'">
										<md-list class="">
											<md-list-item v-for="nom in nominations.data" :key="nom.id"
												...
												<div class="md-list-text-container">
													...
													<span>
														<i class="fa fa-clock-o"></i>&nbsp;<abbr
															:title="nom.created_at|vmUtc|vmLocal|vmDateFormat('lll')">{{nom.created_at|vmTimeAgo}}</abbr>
													</span>
												</div>
											</md-list-item>
										</md-list>
									...
									<md-tab id="nominated" :md-label="nominated.data.length + ' Nominations Given'">
										<md-list class="">
											<md-list-item v-for="nom in nominated.data" :key="nom.id"
											              :href="'/user/' + nom.user.data.url_id">
												<md-avatar>
													<img src="/assets/img/award_win_small.png" alt="award">
												</md-avatar>
							...
							<md-tab id="awards" :md-label="awards.data.length + ' Awards'">
								<md-list class="">
									<md-list-item v-for="win in awards.data" :key="win.id"
										...
										<div class="md-list-text-container">
											...
											<span>
												<i class="fa fa-clock-o"></i>&nbsp;<abbr
													:title="win.created_at|vmUtc|vmLocal|vmDateFormat('lll')">{{win.created_at|vmTimeAgo}}</abbr>
											</span>
										</div>
									</md-list-item>
								</md-list>
