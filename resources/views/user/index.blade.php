...
@section('layout')
    ...
    <user-top class="md-layout md-flex-100" view="{{ $view or '' }}" :user="{{ json_encode(['id' => $user->id, 'url_id' => $user->url_id, 'coverPhoto' => $user->coverPhoto, 'avatar' => $user->avatar, 'fullName' => $user->fullName, 'created_at' => $user->created_at->toDateTimeString()]) }}" :stats="{{ json_encode($stats) }}"></user-top>
